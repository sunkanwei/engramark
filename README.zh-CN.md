...
Engramark 没有需要单独打开的界面。安装后彻底退出并重新打开 Codex 或 OpenCode，然后继续像平时一样与助手对话；不需要记忆特殊命令，也不需要手工调用 MCP 工具。

用下面三步完成第一次体验：

1. 在一个项目中告诉助手：“记住：这个项目默认使用 pnpm，请保存为项目记忆。”助手应当确认保存，并返回记忆编号。
2. 新建一个任务，再问：“查询 Engramark 记忆：这个项目使用什么包管理器？”助手应当找回刚才的记忆并回答 `pnpm`。
3. 测试完成后，可以说：“删除刚才那条测试记忆。”正式记忆只有在你再次确认后才会删除；如果以后可能还要参考，也可以选择归档。

日常还可以直接说“把记忆 18 更新为……”“检查一下有没有候选、长期未使用或可能冲突的记忆”“找一下发布检查清单相关的记忆”。只有你明确要求长期保存时，Engramark 才会写入；普通聊天、临时进度和工具输出不会被记录。

Codex 可以在相关请求中自动获得少量记忆提示；OpenCode 的自动请求雷达默认关闭，但主动查询始终可用。项目记忆与全局记忆的选择、适合保存的内容、候选流程和故障排查见[完整使用指南](docs/使用指南.md)。
